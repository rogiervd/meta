...
struct single_view<Element> : single_view<front, Element> {};

template <class Direction> struct single_view_tag;

template <class... Arguments> struct range_tag<single_view<Arguments...>> {
  typedef single_view_tag<typename single_view<Arguments...>::direction> type;
};
...
template <class Direction> struct empty<single_view_tag<Direction>, Direction> {
  template <class Range> struct apply : mpl::false_ {};
};

template <class Direction> struct size<single_view_tag<Direction>, Direction> {
  template <class Range> struct apply : mpl::size_t<1> {};
};

template <class Direction> struct first<single_view_tag<Direction>, Direction> {
  template <class Range> struct apply {
