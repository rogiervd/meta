...
struct contains : operation::contains<typename range_tag<Range>::type,
                                      Direction>::template apply<Type, Range> {
};
...
template <class Tag, class Direction> struct contains {
  template <class Type, class Range>
  struct apply : mpl::not_<meta::empty<
                     Direction,
                     typename meta::find<Direction, std::is_same<Type, mpl::_>,
                                         Range>::type>> {};
};
