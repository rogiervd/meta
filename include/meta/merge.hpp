...
*/
template <class Direction, class Compare, class Left = void, class Right = void>
struct merge;

// Only direction: insert mpl::less <...>.
template <class Direction, class Left, class Right> struct merge<
...
template <class Compare, class Left, class Right> struct merge<
    Compare, Left, Right,
...
namespace merge_detail {
    ...
    struct left_first
    : mpl::apply<
          Compare, typename meta::first<Direction, Left>::type,
    ...
    struct favourite
    : mpl::eval_if<
          empty<Direction, Left>, mpl::false_,
          mpl::eval_if<
              empty<Direction, Right>, mpl::true_,
...
template <class Direction, class Compare, class Left, class Right> struct merge
: mpl::if_<
...
{
    typedef merge_tag<
        typename merge<Direction, Compare, Left, Right>::direction>
        type;
};
...
namespace operation {
    ...
    // default_direction.
    template <class Direction> struct default_direction<merge_tag<Direction>>
    {
    ...
    // empty.
    template <class Direction> struct empty<merge_tag<Direction>, Direction>
    {
    ...
    // first.
    template <class Direction> struct first<merge_tag<Direction>, Direction>
    {
        template <class Range> struct apply
        : mpl::eval_if<
              typename Range::left_bound,
    ...
    // drop_one.
    template <class Direction> struct drop_one<merge_tag<Direction>, Direction>
    {
        template <class Range> struct apply
        {
            typedef merge<
                typename Range::direction, typename Range::compare,
                // Drop from left if we are left-bound.
                typename mpl::eval_if<
                    typename Range::left_bound,
                    meta::drop<typename Range::direction, typename Range::left>,
                    mpl::identity<typename Range::left>>::type,
                // If not, drop from right.
                typename mpl::eval_if<
                    mpl::not_<typename Range::left_bound>,
                    meta::drop<
                        typename Range::direction, typename Range::right>,
                    mpl::identity<typename Range::right>>::type>
                type;
        };
    };
