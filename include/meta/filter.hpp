...
template <typename Predicate, typename Range>
struct default_direction<filter<Predicate, Range>> : default_direction<Range> {
};
...
struct filter_traits_when_non_empty {
  ...
  typedef meta::filter<Predicate, typename meta::drop<Direction, Range>::type>
      next_filter;
};
...
struct filter_traits<Direction, Predicate, Range, false>
    : mpl::if_<
          ...
          filter_traits<Direction, Predicate,
...
template <class Direction> struct drop_one<filter_tag, Direction> {
  ...
  struct apply<meta::filter<Predicate, Range>> {
    typedef
        typename detail::filter_traits<Direction, Predicate, Range>::next_filter
            type;
  };
};
