...
template <class Type> struct set_element {
  typedef Type key;
};
...
template <> struct empty<set_tag, front> {
  ...
  template <class Void> struct apply<set<>, Void> : mpl::true_ {};
...
template <> struct size<set_tag, front> {
  ...
  struct apply<set<Types...>> : mpl::size_t<sizeof...(Types)> {};
};
...
struct find_in_set {
  ...
  struct apply : index_detail::contains<Element, typename Set::index> {};
};
...
template <> struct insert<set_tag, front> {
  template <class NewElement, class Set>
  struct apply
      : mpl::eval_if<meta::contains<front, NewElement, Set>, mpl::identity<Set>,
...
template <> struct remove<set_tag> {
  template <class Key, class Set> struct apply {
    typedef
        typename index_detail::remove<Key, typename Set::index>::type new_index;
...
struct as_set : fold_reverse<Direction, insert<front, mpl::_1, mpl::_2>, set<>,
                             Sequence> {};

template <typename Sequence>
struct as_set<Sequence>
