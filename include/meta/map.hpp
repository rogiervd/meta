...
template <> struct size<map_tag, front> {
  ...
  struct apply<map<Types...>> : mpl::size_t<sizeof...(Types)> {};
};
...
template <> struct has_key<map_tag> {
  ...
  struct apply : index_detail::contains<Key, typename Map::index> {};
};
...
template <> struct at<map_tag> {
  template <class Key, class Map> struct apply {
    typedef
        typename index_detail::find_entry<Key, typename Map::index>::type::value
            type;
  };
};
...
template <> struct insert<map_tag, front> {
  template <class NewElement, class Map>
  struct apply
      : index_to_map<typename index_detail::insert<NewElement,
                                                   typename Map::index>::type> {
  };
};
