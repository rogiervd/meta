...
template <
    typename Direction, typename Function, typename State = void,
    typename Range = void>
struct scan
{
    typedef scan type;
...
template <typename Function, typename State, typename Range> struct scan<
    Function, State, Range,
    typename boost::disable_if<is_direction<Function>>::type>
: scan<typename default_direction<Range>::type, Function, State, Range>
{};

// Without State.
template <typename Direction, typename Function, typename Range> struct scan<
...
template <typename Direction, typename Function, typename State, typename Range>
struct range_tag<scan<Direction, Function, State, Range>>
{
    // Direction may not be scan <...>::direction because of default
    // arguments.
    typedef scan_tag<
        typename scan<Direction, Function, State, Range>::direction>
        type;
};
...
{
    typedef typename scan<Direction, Function, State, Range>::direction type;
};

template <typename Direction> struct default_direction<empty_scan<Direction>>
{
...
namespace operation {
    ...
    // empty.
    template <typename Direction> struct empty<scan_tag<Direction>, Direction>
    {
    ...
    // size.
    template <typename Direction> struct size<scan_tag<Direction>, Direction>
    {
        template <typename Scan> struct apply
        : mpl::next<typename meta::size<Direction, typename Scan::range>>::type
        {};
    };
    ...
    // first: return State.
    template <typename Direction> struct first<scan_tag<Direction>, Direction>
    {
    ...
    struct drop_one<scan_tag<Direction>, Direction>
    {
        ...
        template <typename Scan> struct apply_not_empty
        {
            ...
            typedef typename mpl::apply<
                typename Scan::function, typename Scan::state,
                typename meta::first<Direction, typename Scan::range>::type>::
                type new_state;

            typedef meta::scan<
                Direction, typename Scan::function, new_state,
        ...
        template <typename Scan> struct apply
        : boost::mpl::if_<
              meta::empty<Direction, typename Scan::range>, apply_empty<Scan>,
              apply_not_empty<Scan>>::type
        {};
    };
