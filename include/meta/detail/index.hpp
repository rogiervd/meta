...
class index<Entry, Rest...> : Entry, index<Rest...> {
private:
...
public:
  ...
  template <class Key, class... PreviousEntries>
  struct remove : rest_index::template remove<Key, PreviousEntries..., Entry> {
  };
...
*/
template <class Key, class Index>
struct remove : Index::template remove<Key> {};
...
struct push_front<Entry, index<Entries...>> {
  static_assert(!contains<typename Entry::key, index<Entries...>>::value,
