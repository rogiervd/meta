...
template <typename... Types> struct return_last_type {
  ...
  wrap<LastType> operator()(swallow<Types>..., wrap<LastType> const &);
};
...
template <> struct empty<vector_tag, front> {
  ...
  template <typename Void> struct apply<vector<>, Void> : mpl::true_ {};
  template <typename... Types, typename Void>
  struct apply<vector<Types...>, Void> : mpl::false_ {};
};

template <> struct empty<vector_tag, back> : empty<vector_tag, front> {};
...
template <> struct size<vector_tag, front> {
  ...
  struct apply<vector<Types...>> : mpl::size_t<sizeof...(Types)> {};
};

template <> struct size<vector_tag, back> : size<vector_tag, front> {};
...
template <> struct first<vector_tag, back> {
  ...
  template <typename... Types> struct apply<vector<Types...>> {
    ...
    */
    typedef decltype(std::declval<vector_detail::return_last_type<Types...>>()(
        0, std::declval<vector_detail::wrap<Types>>()...)) wrapped_last_type;
...
template <> struct drop_one<vector_tag, back> {
  template <typename Vector> struct apply;
  template <typename FirstType> struct apply<vector<FirstType>> {
    typedef vector<> type;
  };
  // Peel initial types off recursively
  template <typename FirstType, typename... Types>
  struct apply<vector<FirstType, Types...>>
      : meta::push<front, FirstType, typename apply<vector<Types...>>::type> {};
};
...
template <> struct fold<vector_tag, meta::front> {
  template <typename Function, typename State, typename Range> struct apply;
  ...
  template <typename Function, typename State, typename Type1,
            typename... OtherTypes>
  struct apply<Function, State, vector<Type1, OtherTypes...>>
      : apply<Function, typename mpl::apply<Function, State, Type1>::type,
              vector<OtherTypes...>> {};

#if !defined(__INTEL_COMPILER) || __INTEL_COMPILER > 1400
  // Optimisation that confuses the Intel compiler.
  template <typename Function, typename State, typename Type1, typename Type2,
            typename... OtherTypes>
  struct apply<Function, State, vector<Type1, Type2, OtherTypes...>>
      : apply<Function,
              typename mpl::apply<
                  Function, typename mpl::apply<Function, State, Type1>::type,
                  Type2>::type,
              vector<OtherTypes...>> {};

  template <typename Function, typename State, typename Type1, typename Type2,
            typename Type3, typename... OtherTypes>
...
struct matching_iterator : mpl::false_ {};
...
*/
template <typename Direction, typename Sequence = void>
struct as_vector : fold_reverse<Direction, push<front, mpl::_1, mpl::_2>,
                                vector<>, Sequence> {};

template <typename Sequence>
struct as_vector<Sequence>
...
class tail_of_longer<meta::vector<Short...>, meta::vector<Long...>> {
  ...
  */
  static meta::vector<> return_after(vector_detail::swallow<Short> const &...);

  template <class After1, class... After>
  static meta::vector<After1, After...>
  return_after(vector_detail::swallow<Short> const &..., After1,
               After const &...);

  typedef decltype(return_after(vector_detail::wrap<Long>()...)) after_holders;
...
template <typename... Types> struct sequence_tag<meta::vector<Types...>> {
  typedef meta::vector_tag type;
};

template <>
struct empty_impl<meta::vector_tag>
    : meta::operation::empty<meta::vector_tag, meta::front> {};

template <>
struct size_impl<meta::vector_tag>
    : meta::operation::size<meta::vector_tag, meta::front> {};

template <>
struct front_impl<meta::vector_tag>
    : meta::operation::first<meta::vector_tag, meta::front> {};

template <> struct push_front_impl<meta::vector_tag> {
  template <typename Vector, typename NewType>
  struct apply : meta::push<meta::front, NewType, Vector> {};
};

template <>
struct pop_front_impl<meta::vector_tag>
...
template <>
struct back_impl<meta::vector_tag>
    : meta::operation::first<meta::vector_tag, meta::back> {};

template <> struct push_back_impl<meta::vector_tag> {
  template <typename Vector, typename NewType>
  struct apply : meta::push<meta::back, NewType, Vector> {};
};

template <>
struct pop_back_impl<meta::vector_tag>
...
template <> struct erase_impl<meta::vector_tag> {
  ...
  template <typename Vector, typename First, typename Last> struct apply_range;

  // Main
  template <typename Vector, typename First, typename Last> struct apply {
    ...
    typedef typename boost::enable_if<
        matches, apply_one_c<Vector, First::index, Last>>::type::type type;
  };
  ...
  struct apply_one_c {
    ...
    static_assert(matches::value, "Iterator must match sequence");
    typedef typename boost::enable_if<
        matches, apply_range_c<Vector, first, Last::index>>::type::type type;
  };
  ...
  struct apply_range_c<meta::vector<FirstType, Types...>, first, last>
      : meta::push<meta::front, FirstType,
                   typename apply_range_c<meta::vector<Types...>, first - 1,
                                          last - 1>::type> {};
};
...
template <> struct insert_impl<meta::vector_tag> {
  ...
  template <typename Vector, typename Position, typename NewType> struct apply {
    typedef meta::matching_iterator<Vector, Position> matches;
    static_assert(matches::value, "Iterator must match sequence");
    typedef typename boost::enable_if<
        matches, apply_c<Vector, Position::index, NewType>>::type::type type;
  };
  ...
  template <typename FirstType, typename... Types, std::size_t index,
            typename NewType>
  struct apply_c<meta::vector<FirstType, Types...>, index, NewType>
      : meta::push<meta::front, FirstType,
                   typename apply_c<meta::vector<Types...>, index - 1,
                                    NewType>::type> {};
};
...
template <> struct insert_range_impl<meta::vector_tag> {
  ...
  struct apply {
    ...
    static_assert(matches::value, "Iterator must match sequence");
    typedef typename boost::enable_if<
        matches, apply_c<Vector, Position::index, NewTypes>>::type::type type;
  };
  ...
  struct push_front_range<Vector, NewTypes,
      ...
      : meta::push<meta::front, typename mpl::front<NewTypes>::type,
                   typename push_front_range<
                       Vector, typename meta::drop<meta::front,
                                                   NewTypes>::type>::type> {};
...
template <> struct end_impl<meta::vector_tag> {
  template <typename Vector> struct apply {
    typedef meta::vector_iterator<typename boost::mpl::size<Vector>::type,
                                  Vector>
        type;
  };
};
...
struct next<meta::vector_iterator<Position, Vector>> {
  ...
  static_assert(
      less_equal<new_position, size<Vector>>::value,
...
struct prior<meta::vector_iterator<Position, meta::vector<Types...>>> {
  ...
  static_assert(
      greater_equal<new_position, mpl::size_t<0>>::value,
      "Attempt to decrement iterator before the start of the sequence.");
  typedef meta::vector_iterator<new_position, meta::vector<Types...>> type;
};

template <typename Position, typename Vector, typename Distance>
struct advance<meta::vector_iterator<Position, Vector>, Distance> {
...
struct distance<meta::vector_iterator<Position1, Vector>,
