...
struct highest_bit_index
    : std::integral_constant<std::size_t,
                             highest_bit_index<(operand / 2)>::value + 1> {};

template <>
struct highest_bit_index<0> : std::integral_constant<std::size_t, 0> {};
...
struct count_pow_2_impl<size_t_vector<half_result...>> {
  ...
  typedef size_t_vector<half_result..., half_result + offset...> type;
};

template <> struct count_pow_2<0> {
...
struct concatenate_counts<size_t_vector<counts1...>,
                          size_t_vector<counts2...>> {
...
struct count_from_zero_impl<end, size_t_vector<bits...>>
    : concatenate_counts<typename std::conditional<
          bool(end &(std::size_t(1) << bits)), count_pow_2<bits>,
          size_t_vector<>>::type::type...> {
  static_assert((end >> sizeof...(bits)) == 0,
                "Internal error: number of bits computed is not sufficient.");
};

template <std::size_t end>
struct count_from_zero
    : count_from_zero_impl<
          end, typename count_from_zero<highest_bit_index<end>::value>::type> {
};
...
struct count_c<begin, end>
    : count_c_detail::check_length<
          typename count_c_detail::add_offset<
