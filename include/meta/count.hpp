...
template <std::size_t end> class count<end> : public count<0, end>
{};
...
template <std::size_t begin, std::size_t end> class count<begin, end>
{
    ...
    {
        typedef meta::vector<
            std::integral_constant<std::size_t, begin>, Rest...>
            type;
    };

public:
    typedef typename compute<typename count<begin + 1, end>::type>::type type;
};
